...
| `scripta-import` | The import skill: the bounded turns that turn an uploaded PDF or DOCX into a universe of its own, so an imported book can afterwards be reviewed, measured, rewritten chapter by chapter and printed like one written here. Specified in `docs/specs/DS014-scripta-import-skill.md`, documented on `docs/scripta-import-skill.html`. |

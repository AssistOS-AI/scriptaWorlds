...
A short `index.md` may link the five views and summarize assessment scope and freshness. It is navigation, not a sixth independent evaluation, but it leads with the same review summary the issues view carries, so a reader meets the strengths, the supported problems and their passages before the metric tables. An empty findings list is never rendered as a clean bill of health: the reading status says whether nothing was evaluated, only part of the selection was evaluated, or all of it was read without a supported issue. Every metric keeps its diagnostics in the default view, including an aggregate's declared weights, arithmetic and calibration qualification, its coverage, its bounds and the reason it is unavailable. Every view must be generated deterministically from the same bundle. If a metric is unavailable, its justification explains the missing prerequisite. A result of zero and an unavailable result must render differently.

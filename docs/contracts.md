...
universes/<universe-id>/
  ...
  turns/NNNN.events.jsonl # the journal of that turn: one JSON event per line, written while it runs (see 2.4)
...
Beside the record, a turn that this server ran has a **journal**: `turns/NNNN.events.jsonl`, one JSON object per line with an `at` timestamp, written while the turn runs and kept afterwards. It holds the queued line with the request, the running line, every phase line, each tool call with its outcome, the text the agent displayed as it worked, and the settle line (`done` with the duration, or `failed` with the message). The journal is what `GET /api/universes/:id/turns/:number/console` renders as the [console](wiki.html#definition-console) of that run. A retry appends to the same journal, so the console of a retried turn lists every session of that turn number in order; a turn that ran before journals existed has no journal, and its `agentLog` is served instead. The journal stops recording raw narration text once it reaches 400 000 characters — the markers (phases, tools, the settle line) continue — and it is written best-effort: a journal write that fails is logged as `[console]` and never fails the turn.
...
// TurnSummary
{"number":4,"kind":"chapter","rewrite":false,"status":"done","createdAt":"…","startedAt":"…","finishedAt":"…",
 "durationMs":211000,"request":"…","chapterNumber":4,"chapterTitle":"The Blind Librarian","error":null,"exports":[]}
...
| GET | `/api/universes/:id/turns/:number/console` | `{"turn":TurnSummary,"console":{"text":"…","live":true\|false,"updatedAt":"…","source":"journal"\|"agentLog"\|"none"}}`: the durable console of that run — everything the agent displayed while it worked, readable while the turn runs and after it settled. `live` is true for a `queued` or `running` turn; `source` says whether the text was rendered from the journal of §2.4 or from the recorded `agentLog` of a turn that ran before journals existed. An unknown turn → `404 NOT_FOUND` |
...
- on `POST /turns` the server immediately writes `turns/NNNN.json` with `status: "queued"` and the request, and starts the journal `turns/NNNN.events.jsonl` with the queued line;
- when the job starts, the record becomes `running` and the journal keeps every event the agent produces (§2.4), so the run is readable live and after the fact;
...
1. **Sticky header**, two short rows, with English labels:
   ...
   - row 2 (book navigation): `‹` followed by one number per slice and then `›`, the three forming one compact group — the numbers sit between the arrows and the arrows never drift apart — and after the group the label of the current slice and the position indicator `6/11`. The numbers are the chapters of the book plus the turns that are running or queued, each carrying the number it will write and marked as in progress.
...
3. **`Edition` dialog** (the `⋯` button): `Download PDF` and `Download DOCX` (shown as `not yet` and disabled until the edition exists), `Generate edition`, a separator, `ALA sessions`, which opens the run console dialog and carries the number of runs in progress as its note, and `New universe`, which returns to the exploration panel of §6.3. There is no `Close`/`Reopen` in the interface; `POST /api/universes/:id/close` and `/open` remain available as API calls only.
4. **The flow**: one slice per chapter, in chronological order. A chapter slice contains the chapter and nothing else:
   - the head: `Chapter N · Title`, with a toolbar of icon controls on the same row — the request, the rewrite, the run that wrote the chapter, reader feedback, a review and the reports;
   - the request control opens the `What you asked for` dialog with the request of the turn that wrote that chapter;
   - the console control opens the `ALA sessions` dialog on the run that wrote that chapter: the run list of the book with the [console](wiki.html#definition-console) of that run selected, live if it is still running, read back from `turns/NNNN.events.jsonl` through `GET /api/universes/:id/turns/:number/console`;
   - the rewrite control opens the `Rewrite chapter` dialog (`POST /api/universes/:id/chapters/:number/rewrite`);
   - the chapter as a book page: title plus rendered text, serif, justified;
   - `running`/`queued` turns appear as their own numbered slice with a live line (`Writing chapter 5…`, `Waiting (1 in queue)`), and the phase the server last published, so the run is watched in the `ALA sessions` dialog; `interrupted`/`error` appear with their message and `Retry`.
   The reader's request is never rendered inside a chapter slice, and neither is the text the agent displayed: both are read from their dialogs.
...
- A single document, no build step, no CDN: `index.html` loads `app.js` as a module, which imports `markdown.js` and the modules under `ui/**`; the ten style sheets under `styles/**` are linked directly from the page, in the order base, reader, live, sessions, explore, composer, overlays, analysis, report, feedback.
...
Roles are `chapter`, `offer`, `canon`, `threads`, `atlas`, `meta`, `design`, `profile`, `annotations`, `corpus`, `rules` and `timing`. A `chapter` entry carries its `chapter` number and the artifact identifier `chapter-NNNN`; an `offer` entry carries `offer-NNNN`; the state roles carry `canon`, `threads`, `atlas` and `meta`. The `rules` role is the registry of rules the host declared for the review (`rules/applicable-rules.v1.json`, whose `schema_version` is `applicable-rules.v1` and whose `registry_version` is `stg-rules.v1`): the rules themselves, the documents each one came from with their hashes, and what was absent with its reason. The charter the registry was derived from travels beside the manifest as that role's own resource at `rules/charter.md`, the way a corpus reference text travels with its manifest, so the evaluator reads the permanent rules of the universe instead of a summary of them. Every declared path is relative, uses forward slashes, stays inside `input/` after symlinks are resolved, and matches its declared `sha256` and `bytes`.
...
| Route | What it does |
| --- | --- |
| `POST /api/universes/:id/assessments` | body `{"phase":"continuity|metrics","mode":"generic|deterministic|supplied","profile":{…},"annotations":{…},"corpus":{"manifest":{…},"files":{"<path>":"<text>"}},"fromTurn":4,"force":false,"scope":{"kind":"book|chapter","chapters":[1,2]},"aggregate":false,"intention":"…","weights":{"cs":0.4,"oi":0.3,"emotional_fit":0.3},"request":"…","brief":"…"}` → `202 {"run":Run}`. A metrics review without a `profile` uses the host's generic profile, and without `annotations` it asks the configured evaluator for them, so ordinary requests need no hand-authored JSON. `request` and `brief` are optional and both are texts: a caller that sends them overrides what the host would otherwise read — the request the selected chapters were written from, or the approved `design.v1` brief of the captured version — and a caller that sends neither leaves the absence recorded with its reason instead of guessed at |
...
| `GET /api/universes/:id/assessments/:runId/console` | the [console](wiki.html#definition-console) of one run: `{"console":{"text":…,"live":…,"updatedAt":…,"source":"journal\|record"}}`, the facts the record kept — its creation, the call in flight, every finished call with its outcome, the continuity substage and the settle line — merged in time order with `generated/evaluator.events.jsonl`, the journal the evaluator child appends to while it works. The same shape a turn's console answers, so a review that needs minutes can be watched while it needs them |
| `POST /api/universes/:id/assessments/:runId` | body `{"action":"cancel"}` → the run is cancelled and a settled one answers `NOT_CANCELLABLE`; `{"action":"retry"}` → an unfinished run (`queued`, `running`, `error`, `cancelled` or `interrupted`) runs the same frozen packet again under the same run id, keeping its attempt records, and a published `done` run answers `409 ALREADY_PUBLISHED`, because a report that was published is not replaced under the reader who read it; `{"action":"delete"}` → the run directory is removed, frozen packet, console and published report together, and a run that is still `queued` or `running` answers `409 STILL_LIVE` (cancel it first) while a report another record names — a newer run that re-evaluates it in `reassesses_run_id`, or a stored reader response whose `run_id` is this run — answers `409 REFERENCED` |
| `POST /api/universes/:id/assessments/:runId/reassess` | body `{"annotations":"reuse|regenerate"}` (default `reuse`) → `202 {"run":Run}`: a `done` run is re-evaluated as a new run over the same frozen packet, whose record names the run it re-evaluates in `reassesses_run_id` and counts one more `evaluation_attempt`. `reuse` re-renders the reading the earlier run accepted, after verifying it against the hash and the source it was saved with, and spends no model call; `regenerate` asks the evaluator for a new reading. A run that is not `done` answers `409 NOT_REASSESSABLE`, and a stored reading that no longer matches what was accepted answers `409 ANNOTATIONS_CHANGED` |
...
`mode` decides who produces the semantic observations — the judgements a model can make about the text: `supplied` means the caller brought an `annotations` document, `generic` (the operator default, set by `ASSESSMENT_MODE=generic|deterministic`) means the host asks the configured evaluator, and `deterministic` means nobody does, so those results stay unavailable with their reason. Supplied annotations and a generated mode are mutually exclusive and answer `400 BAD_MODE`, as does an unknown mode. In `generic` mode the host composes the prompt from what the report skill publishes — the vocabulary at `skills/scripta-metrics-report/schema/annotations.v1.json`, the anchored rubric at `skills/scripta-metrics-report/schema/rubric-anchors.v1.json` (the description of every 0..4 rating, the evidence question of every dimension, its justified exceptions, and the rule that a quiet scene, a static character, a closed ending or a local cultural setting is not a defect by default), the definition, evidence questions and intended-effect qualifications of each of the eight indicators, and a bounded selection of real paired teaching cases in the book's language, chosen by running `skills/scripta-metrics-report/scripts/select-cases.mjs` as a child process, which never returns a case of the declared regression holdout. A missing rubric file or a case selection that cannot be produced fails the run naming the file or the command instead of sending a thinner prompt. The host runs `OMP_BIN` once in the run workspace with the model from `SCRIPTAS_MODEL` and the read-only tool list `read,grep,glob`, each call bounded by `ASSESSMENT_TIMEOUT_MS` (fifteen minutes by default) **per call rather than for the whole run** — a selection read in bounded units pays it once per unit — and validates the answer itself before the phase sees it: the schema version, the source version, evidence that resolves to exact bytes in the frozen packet, and bounds on every field. A rejected answer gets exactly one repair call whose prompt names the fields that failed; a process failure — a nonzero exit, a termination signal, a timeout, a spawn failure or an answer past the byte budget — is not a schema failure, so it ends the stage with the process reason instead of spending the repair on output that cannot be repaired, and nothing is published from it. The answer stream is bounded while it arrives and the prompt is bounded before the child starts. The answer is read out of whatever the evaluator said: a document wrapped in prose, fenced, or followed by further chatter is taken (the host scans for the balanced object rather than trusting the first and last brace), while an answer that begins a document and stops before closing it is recorded as `truncated` — the repair call then asks for a smaller document — and an answer with no document at all is recorded as `no-object`. The prompt states the bounds the host enforces (items, segments, findings, quote length and total size) so that an answer is not cut off by the evaluator's own output limit. The document it accepts is stored at `generated/annotations.json` beside the run and passed to the phase by absolute path, so the frozen packet keeps exactly the bytes the review was asked about and `packet_intact` records whether it still hashes to them. The model work happens **inside the run, not inside the request**: `POST /api/universes/:id/assessments` captures the packet, writes the run record and answers at once with the run `queued` (or already `running`), and the annotation stage runs before the phase, so a caller never holds an HTTP request open for the two minutes an evaluator may take and the interface can show the run's state from the first moment. A judgement may only rest on the text the run was asked about: evidence is verified against the packet bytes and against the selection, a claim whose evidence lies outside it is refused with `EVIDENCE_OUT_OF_SCOPE`, and a claim whose evidence lies entirely in declared context chapters is demoted to an unavailable result with its reason rather than scored. The stage accepts an answer only when it is a readable review: the document must dispose of every component the call asked about — each of the four judged metrics and each indicator of the published vocabulary carries either a judgement or an explicit reason it could not be assessed, and a document that disposes of nothing is refused, while an honest `not_assessable` with a reason is a valid answer. Evidence is checked byte for byte against the frozen packet, its offsets must fall on UTF-8 character boundaries, and a passage that also occurs elsewhere in the file is evidence at the occurrence it declares rather than an ambiguity. A model answer may instead name the line a passage is on and quote it — `line` plus `quote`, which is what the prompt asks for, because a model reads the packet through a paged view and cannot count bytes — and the host resolves that pair itself: it searches inside the bytes of the lines the item names (extended by the newlines the quote carries), refuses a quotation that does not occur there and one that occurs there more than once, writes the resolved `start` and `end` into the item with `offsets_resolved_by: "host"`, and counts the resolutions in the attempt record as `evidence_resolved`. A document that declares offsets has them verified exactly as before, so a caller that can count bytes keeps the strict form. A model that answers with no usable document ends the run as an `error` whose message begins `annotation stage:` — the record keeps every attempt, its exit code, its answer length and the validation errors, and no phase runs and no file is published, instead of a report whose judgements are missing. A retry of such a run, or of a run whose phase failed after the observations were accepted, reuses the accepted document and does not pay for another reading — and only that document: the stored reading is verified against the hash and the source it was accepted with, so a document that changed, or that names another version, fails the run with `annotation stage:` and the reason instead of being published as a reading it never was. The metrics phase itself never calls a model.

The inputs a phase declares are frozen into the packet and passed to it explicitly: `annotations` to both phases, a `corpus` to the metrics phase, a `profile` to the metrics phase. An input a phase does not consume answers `400 UNSUPPORTED_INPUT` instead of being ignored. A declared corpus must arrive with the text of every reference it names — captured beside its manifest, so the review can read what it compares — and a manifest without that text answers `400 MISSING_RESOURCE`; a supplied text whose bytes do not match the hash its reference declares answers `400 HASH_MISMATCH` before any phase runs. The packet's own inventory lists the declared files; the corpus reference texts travel beside the manifest, which is the artifact of role `corpus` and declares each text's hash, recomputed by the review. A reference may also declare the corpus source it was copied from as `source: { id, version }`, and that declaration is what the report reads: a reference is the candidate's own copy only when it declares the packet's own universe and accepted version, so two texts that happen to be identical byte for byte stay independent references unless their declaration says otherwise. The host enforces the shape at the boundary — a `source` that is not exactly `{ id, version }` with both values non-empty answers `400 INVALID_CORPUS` before any packet is captured, because half of an identity cannot be verified and is never completed from a hash. What the manifest declared is recorded on the run as `corpus_references`, beside the `corpus_candidate_identity` the references are compared against, so a reader of the record sees which source each reference claimed.

There is no rule to invent on either side. The host freezes the applicable rules into the packet before any evaluator exists, from resources it can verify: the charter of the universe, an approved `design.v1` brief for the captured version (its `world_assumptions` of `epistemic_kind` `law` or `social_rule` become rules of source `request`, named `brief-<assumption id>`), the directions a human accepted in an approval of that version and the directions a writing turn carried into the chapter it wrote (`direction-<8 hex>`, source `request`), and the general rule set the report skill publishes at `skills/scripta-metrics-report/schema/rules.v1.json` (source `stg`). The evaluator reports an outcome per (rule, output) pair and never restates a rule: an answer carrying `requirements.rules`, a registry version or an aggregation policy is refused, and an outcome naming an undeclared rule, an output outside the reading or a duplicate pair is refused with the reason. A document that does not exist is not a violation: a request, a brief or an intention that was never recorded is listed in `missing` with its reason, no rule is derived from it, and the compliance view states the absence, which is what an imported book shows.

A run is identified by a fingerprint (`input_fingerprint`, version `assessment-input.v2`) over the phase, the accepted version, the normalized requested scope (its chapters, segments, arcs, context and omitted material — a scope naming a chapter the captured version does not have is refused with `400 BAD_SCOPE` before any run exists), the trigger and its arc, the mode that produces the observations, the evaluator model, the identity of the prompt material (the prompt version plus the hashes of the published annotation vocabulary, the anchored rubric, the teaching-case index and the case-selection command the prompt is built from), the words the review was asked about (`request`, `brief`, `intention`), the profile hash, the annotation hash, the corpus manifest hash and the hash of every captured resource. The published bundle carries the same material for a reader that does not have the run record: `provenance.resources` names every input the run read with the sha256 and byte count of the exact bytes, `provenance.annotations.sha256` identifies the generated judgement document, `provenance.evaluation` hashes the skill's published vocabulary, rubric anchors, study schema and case index and lists the evaluator labels, and `provenance.evaluator_provenance` carries the host's own `evaluator_provenance` object verbatim with `provenance.evaluator_provenance_sha256` over its canonical JSON. A generic profile carries no wall-clock field, so two requests that ask for the same review produce the same profile; the identity prefix changes with the field set, so a record written under an older one never claims to be this review. A deterministic run and a model run over the same selection are two different reviews. Two requests with the same fingerprint are the same assessment and the second returns the first with `deduplicated: true`; requests are created under a per-universe lock so simultaneous identical ones converge on one record; a record written before fingerprints existed carries none and is never treated as a duplicate. The host says `requested|arc` and the report bundle says `request|arc`; the mapping is explicit here and in the run record.
...
`run.json` is the durable record of one run: `run_id` (independent of turn and chapter numbering), `universe_id`, `phase`, `trigger` (`requested` or `arc`), `arc_id`, `version`, `scope`, `requested_scope` (kind, chapters, segments, arcs, context chapters and what is omitted, resolved against the captured version), `profile_source` (`supplied`, `generic` or `none`), `profile_sha256`, `annotation_mode`, `annotation_model`, `annotation_prompt_version`, `in_flight` (the call the evaluator is answering right now — its attempt and unit, the model, the prompt file and hash, the deadline, the child's pid and when it started — written while the call runs and cleared when the runner returns, so `running` is never the whole story of a review that takes minutes), `annotation_attempts` (one entry per call, with its own `attempt_id`, its prompt file and prompt hash, the child's pid, its exit code, the validation errors, its stderr, the answer length, its timing, and the provider usage only when the provider reported one), `annotation_errors`, `annotation_prompt_sha256`, `annotation_resources` (the published path and hash of the vocabulary, the rubric anchors, the case index and the case-selection command the prompt was built from), `annotation_provenance` (the invocation manifest described below), `annotation_evaluator_id`, `generated_annotations`, `annotations_sha256` (the hash of the bytes of `generated/annotations.json`, which is written pretty-printed, so a reader who re-renders it compares the same bytes), `reused_annotations`, `packet_intact`, `output_boundary`, `write_boundary`, `result_revision`, `result_files`, `published_at`, `reassesses_run_id`, `evaluation_attempt`, `continuity_source`, `corpus_references` (the `{ id, path, sha256, source }` of every declared reference, with `source` null when the manifest declared none), `corpus_candidate_identity` (the `{ id, version }` the references are compared against: the packet's own universe and accepted version), `inputs`, `resources`, `corpus_manifest_sha256`, `input_fingerprint`, `status` (`queued`, `running`, `done`, `error`, `cancelled`, `interrupted`), timestamps, `exit_code`, `error`, `outputs`, `attempts` and the captured `envelope`. One lifecycle controller owns every run while it executes: the annotation stage, the repair call and the phase are all reachable through it. A cancellation stops the active child, escalates to `SIGKILL` when the child ignores its signal, marks the executing record itself (a late success cannot overwrite it), publishes nothing, and refuses a retry while the child still lives; a server shutdown stops every live review child before it releases the store. A run that ends with a nonzero exit is `error` and names the reason the phase printed; a run that was still `queued` when the process stopped resumes at the next start with the same `run_id`, and one that was `running` becomes `interrupted` and is retried over the same frozen packet. No run ever writes inside `universes/`, so a failed assessment cannot roll back a chapter.

Every `generic` run also keeps the evaluator's provenance, so that another session can establish what the evaluator saw and which judgement produced each result. `generated/prompt-attempt-<n>.txt` holds the exact prompt of each attempt and `generated/prompt.txt` the accepted one; their hashes are `annotation_prompt_sha256` and each attempt's own `prompt_sha256`, and the offsets of the accepted attempt are what the run's report refers to. `annotation_resources` names the published files the prompt was built from with their hashes, so a change to the vocabulary, the anchored rubric, the case index or the selection command changes the run's identity. `annotation_provenance` is the `evaluator-provenance.v1` manifest: the prompt with its version, hash, size and file; the resource hashes; the teaching cases that were shown and the declared holdout that was not; the selection, the reading list and the packet; the settings the evaluator was launched with (model, timeout, attempts allowed, the tool list, `no_lsp` and both byte bounds); one entry per attempt with its own identity; the labels the document declared for itself; and the provider usage when the provider reported one, which is `null` rather than an invented number. Alongside it, `generated/provenance.json` holds the hash and size of every artifact the stage wrote, computed from the bytes on disk rather than from a re-serialization. The evaluator identity is `evaluator_id`, derived by the host from its own invocation — `host:` plus the hash of the prompt version, the model, the accepted prompt and the resource hashes — not from the `evaluator` label the model wrote; `declared_evaluator_labels` records those labels beside it and `evaluator_labels_match` says whether they name the model the host actually launched. The accepted document carries the same object as its own `evaluator_provenance`, and the metrics phase copies it into the bundle's `provenance.evaluator_provenance`, so the portable report names its evaluator and its prompt material without trusting the run workspace. While the evaluator works, the host appends its stream to `generated/evaluator.events.jsonl` beside the run — the same vocabulary a turn's journal uses, the text the agent narrates and each tool call with its outcome — so `GET /api/universes/:id/assessments/:runId/console` can render what the agent is reading while it reads it, live and after the run settled.

The reading itself is planned. A selection is planned as bounded units before any call is made — one unit per chapter, or one per byte range when a single chapter exceeds the per-unit budget, each with the explicitly permitted context chapters and, for a range, the rest of its own chapter — and the plan is written to `generated/units/plan.json` with the limits it was made under (`ASSESSMENT_UNIT_BYTES`, `ASSESSMENT_MAX_UNITS`, `ASSESSMENT_TOTAL_BYTES`), so the bound is a file a reader can open rather than a claim in a prompt. Each unit is read by its own evaluator call, validated against the packet on its own, and kept as `generated/units/<unit id>.json`; a synthesis call then assembles the selection's conclusions from those verified observations, and per-unit grades are never averaged into a verdict. A unit the plan could not fit is `omitted` and the reading says so: `annotation_plan`, `annotation_units` and `annotation_reading` record the units attempted, completed, omitted and failed, `reading.complete` is true only when every declared unit completed, and the same record travels into the published bundle through `evaluator_provenance`. A unit that cannot be read fails the run — nothing is published from a partial call — and a retry resumes the units whose artifacts validate against the packet instead of paying for them again.

A published result is immutable. `retryAssessment` refuses a `done` run with `409 ALREADY_PUBLISHED`, and a re-evaluation is a new run over the same frozen packet: `reassessAssessment` copies the packet instead of capturing the live book again, so the new run answers the same question about the same text, and its record names the run it re-evaluates in `reassesses_run_id` while the earlier record and its published bytes are left exactly as they were. A `done` run records the revision a reader's reaction binds to: `result_files` carries the name, hash and size of every published file and `result_revision` is a hash over them. Nothing is published by a run that fails, and the attempt history is history: a retry and a re-evaluation append their calls to `annotation_attempts` instead of erasing the calls before them.

A review writes inside its own run directory and nowhere else. The published set of a run is what the host found as plain files in its `result/` directory: an entry that is a symbolic link, or that is not a plain file name, is refused and the run publishes nothing, which `output_boundary` records. After every child has terminated the host also compares the version directory and the universe workspace with what they held before the run started, and a file that appeared where only run directories, arc events and feedback belong is named in `write_boundary` and fails the run. The frozen packet is re-hashed after the children — including a file inside it that no declaration names — so `packet_intact` describes the bytes on disk rather than the text of the manifest. The host cannot sandbox a child process, which is why the evaluator gets a read-only tool list and why these checks exist: they refuse the result instead of claiming a boundary that was never enforced.

A metrics phase does not have to be given a continuity result to report one. The host runs the continuity skill itself, as a separate deterministic substage over the run's own frozen packet, into `continuity/` inside the run directory, and the result it publishes is bound to the same version and scoped to the same selection: the population is narrowed to the reviewed chapters and the counts are recomputed from the comparison records that lie inside them, with a comparison that cannot be attributed to any chapter of the selection reported as unattributable rather than folded into the rate. What the substage found travels to the metrics phase as the `continuity` member of the annotations document the host hands it (in `generic` mode beside the generated observations, in `deterministic` mode as an annotations document that carries the continuity result and the registry and no observation at all), and the scoped document stays at `generated/continuity.json` with the substage's own result beside it. A substage that refuses the packet, times out or exits non-zero is recorded in `continuity_substage` with its reason and the review still publishes: continuity is an input, never an acceptance gate on a literary reading.

A continuity result supplied with a review is bound to the packet it came from before anything is recorded about it. Its authoritative source identity is `annotations.continuity.source_version`, with the legacy `version` field read only as a fallback when `source_version` is absent; a result that names both differently, or names neither, or names a version other than the captured one is refused with `400 BAD_CONTINUITY_SOURCE`. Its population is read as the report reads it — `scope.chapters_reviewed`, else `scope.chapters`, else a top-level `chapters`, with `scope.omitted` — and a chapter outside the captured version, or a population that does not cover the chapters this run selects, is refused with `400 BAD_CONTINUITY_SCOPE`. What was accepted is recorded in `continuity_source` (`declared_field`, `source_version`, `population`, `omitted`, `scope_declared`, `sha256`), and that binding, not the raw document, is the continuity input the run's `input_fingerprint` covers.

Two runs are the same assessment when their `input_fingerprint` values are equal, and that is what a second request compares before it writes anything: it answers with the run that already exists and `deduplicated: true` instead of reviewing the same evidence twice, while a request that changes any effective input — the scope, the mode, the evaluator, the prompt material, the request, the brief or the intention — is a different review that the scan does not suppress. An explicit re-evaluation keeps the same fingerprint and is still a new run, because a caller asked for one rather than relying on the duplicate rule. Findings an author selects travel into one later rewrite as input data of that request (§4.3) together with the version they were reported against, so stale evidence is refused instead of applied and the revision stays a single candidate pass.
...
<workspace>/<universe-id>/feedback/
  ...
  comparisons/<session-id>/session.json   one A/B session over two targets, `reader-feedback-comparison-session.v1`
  comparisons/<session-id>/response.json  the preference recorded in that session, `reader-feedback-comparison-response.v1`
```
...
**A target** is a book (or an arc, or a chapter) as one reader saw it, frozen when the reader opened the form, and it is the only thing the text of a response is anchored to. `target.json` carries `schema_version` (`reader-feedback-target.v1`), `target_id`, `universe_id`, `source_version` (the accepted version the reader was shown), `scope` (`kind`, `chapters`), `language`, `created_at`, `displayed` (`hash` of the text bundle, `files` with their `path`, `sha256` and `bytes`), `chapters` (number, title, `sha256` of each chapter file as displayed) and `questionnaire`. A target carries only what every reader of that version and scope shares — the frozen text and the questions; the report a reader was looking at, the findings of it they saw, their own note and what they had already seen belong to that reader's response, so a later reader inherits no report association they never had. A target is written once and never rewritten; a second session against the same version and scope returns the same target instead of a new one. A capture is bound to the version the reader read: the body may carry `sourceVersion`, the accepted version that was on screen, and `displayed` (`{"chapters":[{"number":1,"sha256":"…"}]}`), the hash of every chapter of the scope that reader displayed. When the accepted version has moved on, or a displayed hash is not the accepted text of the chapter it names, nothing is frozen: the call answers `409 STALE_TARGET` and `error.details` carries `requested_version`, `accepted_version`, `scope` and — for a hash — `chapter`. The frozen copy of the requested version and scope is reopened by its own identity instead, answering `200` with `deduplicated: true` and `reopened: true`. The accepted version is read again after the text is copied, so two versions can never be frozen under one identity while a turn publishes. The frozen text is what makes a response reopenable years later: no quotation from a response is ever applied to prose that changed, and a target whose text cannot be reproduced answers `STALE_TARGET` rather than attaching a silent reading.

**The questionnaire** is owned by the backend and frozen into every target, so a response is read against exactly the questions its reader saw: `questionnaire.version` and `questions`, each question with `id`, `label`, `low`, `high` and its `options` (the numbers 1 to 5). The questions are `interest`, `clarity`, `voice`, `emotion` and `continue`. A response's `questionnaire_version` must equal its target's, an option must be one the target declares (the number or its string form), and `null` means the reader skipped that question — a skipped answer stays visible as skipped and is never counted as a low score. A `target_id` is deterministic: the first 16 hexadecimal characters of the hash of the universe, the accepted version, the scope kind and the chapters, which is what makes a second session against the same version and scope reopen the same target instead of freezing another copy. The same frozen copy declares the reactions a response may add, under `questionnaire.reactions`: each has `id`, `about` (`report` or `text`), `label`, `low`, `high` and its `options`. `usefulness` is about the report the reader was looking at, declared 1 to 5; `defect_present` is about the text itself, declaring `no`, `unsure` and `yes`. A reaction is optional, carries its own `comment` of at most 1 200 characters, and is refused with `400 INVALID_FEEDBACK` when the questionnaire does not declare it, when it names an option that reaction does not declare, or — for a reaction whose `about` is `report` — when the response names no report.

**A response** is `reader-feedback.v1` and carries: `schema_version`, `feedback_id` (issued by the server), `target_id`, `universe_id`, `reader_id`, `reader_kind` (`team_human` for a real submission, `model` for an annotation, `synthetic` for a fixture), `questionnaire_version`, `created_at`, `accepted_source_version`, `scope`, `language`, `run_id` and `finding_ids` when the reader was looking at a report, `note` (the reading session's own note, at most 200 characters), `answers` (each answer is `null` or a value from the questionnaire's declared options, with its own `comment`), `comments` (free text, with `evidence` ranges quoted from the target's frozen files), `conditions` (`where` the reader read, how long, on what device, and `exposure` — whether they had already seen `model_scores` or `other_comments` — each recorded when declared, `null` when it was not, never invented), `reactions` (`usefulness` and `defect_present`, each `null` or `{"value":…,"comment":…}`), and `revision_of` (`feedback_id` of the response this one corrects, or `null`) plus `revision` (an integer starting at 1). A correction is a new entry that points at the one it replaces; nothing is edited in place, so the history of what a reader said is the history of files.
...
**What a response points at.** `run_id` names a published report of this book and `finding_ids` the findings of it that reader saw. A report of another version, a report whose scope covers a chapter the target does not display, a finding that report never published, and a finding named with no report are each refused with `400 INVALID_FEEDBACK` (or `404 NOT_FOUND` for a report this book does not hold) and nothing is written, so a reader can never be recorded as having seen an association they could not have had. Because the report belongs to the response rather than to the target, two readers of one frozen target answer it with their own sessions — one after reading a report, one without — and the `exposure` declarations are what keeps an unaided reading apart from a report-assisted one. Only the frozen text and the questionnaire are shared by the readers of a target.
...
**What a listing reports.** `GET /api/universes/:id/feedback` answers the responses, the targets, the counts of that book, and `accepted_version` — the version the store holds right now, `null` while a turn is writing — which is what a reader's form sends back with the text it displayed. The counts separate provenance: a response whose `reader_kind` is `model` or `synthetic` is never added to the number of team readers or team responses, because the difference between what a reader said and what a model annotated is the whole point of keeping both.
...
**The summary (`reader-feedback-summary.v1`)** is the same evidence stated honestly, at `GET /api/universes/:id/feedback/summary` (`--summary` on the command above prints it too). It reports how many independent readers answered, and per question, per **population** and per provenance the number of readers, the answers, the skips and the readers who left the question unmentioned, and the **distribution of every declared option** — never a mean alone. A population is the accepted version, the scope the reader was shown, the language and the questionnaire version taken together: a chapter rating is never added to a book rating, an answer given on a changed questionnaire scale is never added to the answers of the old one, and a version whose figures pool several populations carries `pooled_across_targets: true` with the populations it pooled beside the per-population figures that should be read instead. The unit of every distribution is the independent reader and never the response record: for each question the latest current answer of each reader inside the population counts once, `earlier_answers` and `unmentioned_records` say what was submitted before it and what never mentioned the question, and a reader who submitted three times is one reader — `responses` is therefore counted in readers, `records` counts files, and the two do not have to be equal. Each question is declared per questionnaire version (`questions[].questionnaire_version` and `declared_by`), so a questionnaire that changed a label or a scale declares a question of its own and two scales are never collapsed into one row, and an answer outside the scale it declares is counted under its own value in `undeclared`. A mean is reported beside its distribution from three independent readers on, and below that the place of the mean carries the reason instead of a number. Nothing is added across the three kinds: a model's annotation or a synthetic fixture has its own numbers in every question, and the number of readers counts only real responses. Nothing is added across versions either: every number about answers — the responses, the answers, the skips, the unmentioned ones, the distribution and the mean — lives inside the accepted version it was written against, which `versions` names one by one, and a question no response answered is reported as zero answered with the responses that left it unmentioned. The book-level `counts` and a reader's own totals count records and answers, never a rating added to another version's. `historical` marks the versions that are no longer accepted. A correction is not counted twice: the response it replaces is `superseded`, a withdrawn response has no body left, and both are reported apart from the answers that count.

The summary **refuses** three claims with the reason it refuses and the counts that would be needed, and it states no number for them: a **comparison** between two accepted versions of the book (the answers are ordinal ratings given by different readers to different texts; the summary reports the two versions side by side and refuses the ordering, and while a version holds fewer than three independent team readers who answered the question it says the comparison is not supported at all), a **trend** (the store holds readings of a text, not repeated measurements of the same readers over time), and a **claim about what the team thinks** (the store records who answered, never who read the book and did not answer). Where it cannot know something, it says so in place of the number: a reader who answered one question of five is listed with what they answered and what they skipped, a target that holds a single response carries a note that nothing more can be read from it, and what the withdrawn, superseded or deleted records no longer count is stated in `limitations`. An unknown book answers `404 NOT_FOUND`, and a book whose readers have not responded yet answers `404 NO_FEEDBACK` for both the dataset and the summary; neither writes anything.

**The comparison (`reader-feedback-comparison.v1`).** One question, two accepted versions of the same book, and the rule the summary already states: `GET /api/universes/:id/feedback/comparison?question=interest&from=sha256:…&to=sha256:…` answers `{"comparison":Comparison}` while each side holds at least three independent team readers whose counted answer to that question exists. The two sides are read from **comparable populations**, which share the scope, the language and the questionnaire version: a chapter reading is never put beside a book reading and a rating taken with another questionnaire scale is never put beside this one. When a version holds several comparable populations for the question the comparison reads the book-wide one (or the first in reading order), names that choice in `basis.selection` and lists the others, with their counts, in `basis.other_populations`, instead of adding them together. Each side reports the `population` it was read from, its `records` (the current response files) and its `provenance` (the counted readers per kind, with the same numbers the summary states for that population). When the two versions hold no population read with the same scope, language and questionnaire the route answers `409 NO_COMPARABLE_POPULATION`, naming the populations each side holds for the question and stating no distribution, no mean and no difference. The document names the question with the questionnaire that declared it, states its `basis` — the rule, the bound, that only current responses are counted (a withdrawn response, or one a correction replaces, is not) and that the three provenances are kept apart — and reports the two sides side by side: for each side the accepted version, whether it is `historical`, the `records` per provenance, and per provenance the responses, the answers, the skips, the responses that left the question unmentioned, the whole distribution of every declared option and the mean beside it (from three answered responses on, with the reason in its place below that). Each side also names the identities behind it: `readers` carries the team readers who answered it, `annotations` the model annotations and synthetic fixtures, each with its `reader_kind`, so what was said is never separated from who said it. The comparison states **no ordering and no difference**: `ordering.difference` is `null` and `ordering.reason_code` is `ORDINAL_RATINGS`, the refusal the summary reports, because the answers are ordinal ratings given by different readers to different texts. The two sides are the versions the caller named, in the order it named them; the store does not turn two content identities into a sequence and invents no chronology. While a side holds fewer than three independent team readers who answered the question the route answers `409 COMPARISON_NOT_SUPPORTED` with the reason the summary states for that question — or, when the summary reports none, the same shape of sentence — the counts that would be needed, and no distribution, no mean and no difference anywhere. A version that is not a version of this book — another book's, or one the store keeps no text and no response of — answers `409 COMPARISON_UNKNOWN_VERSION` naming the versions it does know, and a request that does not name one question and two different versions answers `400 BAD_COMPARISON`.

**The self-contained dataset (`reader-feedback-dataset.v1`).** The lightweight dataset above stays where the prose was frozen; when an analysis has to outlive the store, `node scripts/export-feedback.mjs --universe <universe-id> --dataset [--out <file>]` and `GET /api/universes/:id/feedback/dataset` produce a snapshot that carries the evaluated text itself: the `universe_id`, the `book`, the questionnaire definitions, the `readers`, the `targets`, a `texts` entry per target whose `files` carry every displayed file byte for byte (`path`, `name`, `sha256`, `bytes`, `text`, historical versions included), the `responses` with their answers, comments and conditions, the `selection` (`counted`, `superseded`, `withdrawn`, `corrections` and the rule that decides them) and a `manifest` with the counts and the hashes of every part, including the hash of the lightweight export of the same store. Every frozen file is read back from its own target and checked against the `sha256` and the byte count the target recorded before the snapshot is written, so a file that no longer reproduces them answers `409 STALE_TARGET` and no snapshot is produced. Readers appear under a stable pseudonym (`reader-p…`, derived from their identifier inside that book, which is what keeps the repeated readings of one person visible as one person) and the display names and identifiers travel only in the internal snapshot, which must be asked for by name — `--internal` on the command, `?identity=internal` on the route; the pseudonymous one is always the default. The command writes the same bytes as the route, the same store yields the same bytes, and nothing is written anywhere. An unknown book answers `404 NOT_FOUND`, a book with no response `404 NO_FEEDBACK` and an identity mode the store does not know `400 BAD_DATASET`.

**The comparison session (`reader-feedback-comparison-session.v1`, `reader-feedback-comparison-response.v1`).** A comparison of two existing distributions is descriptive evidence; a session is an observation the team records on purpose, and it needs no population — one member of the team is enough to open one. `POST /api/universes/:id/feedback/sessions` with `{"question":…,"target":…,"otherTarget":…,"readerId":…}` answers `201 {"session":Session}` over two different immutable targets of the same book that declare that question with the same options. A question the two targets do not share, a scale mismatch, the same target twice or a malformed identifier answers `400 BAD_SESSION`; a target or a reader this book does not have answers `404 NOT_FOUND`; an identity that asked for its deletion answers `409 READER_DELETED`; a repeated call for the same session answers the record that exists with `deduplicated: true`, and a different pair or reader under the same identifier answers `409 CONFLICT`. The session carries its `reader`, the `question` (identifier, label, ends, options, questionnaire version), `same_questionnaire`, `targets` (`A` and `B`, each with `target_id`, `accepted_version`, `historical`, `scope`, `language`, `questionnaire_version`, `displayed_hash`, its displayed `files` and its `chapters`), `display` (`order`, the side drawn first, `drawn_from` and the note that a refresh reads the stored order) and `measurement` (`claimed: false` with the reason that a recorded session is an observation and the store claims no effect and no reliability from it). The display order is drawn once, from the session's own identifier, when the session is created and stored: `GET /api/universes/:id/feedback/sessions/:sessionId` returns the same order every time. `POST` on that route with `{"preference":"A"|"B"|"tie"|"unable","rationale":…,"ratings":{"A":…,"B":…},"conditions":{…}}` answers `201 {"session":Session}` and writes the response: the preference, the rationale (at most 4 000 characters), the reader's own ratings of the two sides when they give them (declared options or `null`), and the conditions they read under (`where`, `duration_minutes`, `device`, `read`, `seen_before` — which of the two they had already read — and `saw_report`). A tie and an `unable` are answers of their own and are counted as such. One session holds one answer: an identical retry answers the record that exists (`deduplicated: true`), a different one answers `409 CONFLICT`, a preference that is not one of the four answers `400 BAD_SESSION`, and an answer naming another reader than the session's answers `400 BAD_SESSION`. `GET /api/universes/:id/feedback/sessions[?target=…]` lists the sessions of the book, or of one target, each with its `response` and its `counts`: the sessions opened and the answers recorded for that pair of texts, `orders`, `by_position` (left, right, tie, unable — where the reader saw it) and `by_role` (A, B, tie, unable — the role the caller named), reported apart because the display order is drawn per session and a choice of the text on the left is not the same observation as a choice of role A. These are counts of recorded observations; the ordinary descriptive distributions keep their own counts beside them, a small population never prevents the team from recording what it saw, and nothing here is presented as a measurement of a difference.
...
**Ownership and errors.** Feedback is written by the server, never by the agent, and never inside `universes/`. An unknown target answers `404 NOT_FOUND`; a response for a target whose version is no longer accepted is accepted and marked `historical: true` in the listing rather than refused, because a reader's opinion of an older text is evidence about that text; a response naming an unknown reader answers `404 READER_NOT_FOUND`; a second submission with the same `feedback_id` and the same content answers `200` with the existing entry (`deduplicated: true`), while a different content under the same identifier answers `409 CONFLICT`; a withdrawn response stays readable with `withdrawn: true` and an empty body of answers; a correction has exactly one lineage, because its parent, that parent's reader, target, questionnaire, source version and withdrawal state, and the absence of another correction are validated inside the same serialized transaction as the write — two simultaneous corrections of one response cannot both commit, the second answers `409 CONFLICT` naming the correction that exists, an identical retry of the one that committed stays `deduplicated: true` instead of writing twice, and a correction naming another target, questionnaire or version than its parent answers `400 INVALID_FEEDBACK` rather than moving an answer. Reading the dataset or the summary of a book that does not exist answers `404 NOT_FOUND`; reading either of a book that holds no response answers `404 NO_FEEDBACK`.

**Routes.**

| Route | What it does |
| --- | --- |
| `POST /api/universes/:id/feedback/targets` | body `{"scope":{…},"sourceVersion":"sha256:…","displayed":{"chapters":[{"number":1,"sha256":"…"}]}}` → `201 {"target":Target}`, `200 {"target":Target,"deduplicated":true}` for the version and scope already frozen, `200 {…,"reopened":true}` when the frozen copy of the version the reader read is reopened; `409 STALE_TARGET` with `details.requested_version`, `details.accepted_version` and `details.chapter` (for a displayed hash) instead of freezing prose the reader never saw |
| `GET /api/universes/:id/feedback/targets/:targetId` | one target, with its frozen files listed |
| `GET /api/universes/:id/feedback/targets/:targetId/text/:name` | the frozen bytes of one displayed file (`text/markdown; charset=utf-8`), re-checked against the hash the target recorded; `404 NOT_FOUND` for a file it does not display, `409 STALE_TARGET` when the frozen copy no longer reproduces its hash |
...
| `GET /api/universes/:id/feedback/dataset` | the self-contained snapshot (`reader-feedback-dataset.v1`) with the frozen prose inside it, served as a download (`<universe-id>-feedback-dataset.json`, or `<universe-id>-feedback-dataset-internal.json` when `?identity=internal` asks for the one that carries identities); `409 STALE_TARGET` when a frozen file no longer reproduces the hash its target recorded |
...
| `POST /api/universes/:id/feedback/sessions` | body `{"question":…,"target":…,"otherTarget":…,"readerId":…}` → `201 {"session":Session}`; opens one A/B session over two immutable targets and draws its display order once; `200` with the existing session for a retry of the same one |
| `GET /api/universes/:id/feedback/sessions` | `{"sessions":[…]}` oldest first, each with its `response` and the counts of its pair of texts, optionally `?target=…` for one target |
| `GET /api/universes/:id/feedback/sessions/:sessionId` | one session with the display order it stored, its `response` and its `counts` |
| `POST /api/universes/:id/feedback/sessions/:sessionId` | body `{"preference":"A"\|"B"\|"tie"\|"unable","rationale":…,"ratings":{…},"conditions":{…}}` → `201 {"session":Session}` with the recorded answer; `200` for an identical retry, `409 CONFLICT` for a different one |
...
| `POST /api/universes/:id/feedback` | body `{"targetId":…,"readerId":…,"answers":[…],"comments":[…],"conditions":{"where":…,"exposure":{"model_scores":true,"other_comments":false}},"reactions":{"usefulness":{"value":4,"comment":…},"defect_present":{"value":"yes"}},"run_id":…,"finding_ids":[…],"note":…}` → `201 {"feedback":Response}`, or `200 {…,"deduplicated":true}` for an identical retry |
...
`targets`, `export`, `dataset`, `summary`, `comparison`, `sessions` and `revisions` are reserved segments of the feedback route: a response whose identifier is one of those seven words is not readable through `GET /api/universes/:id/feedback/:feedbackId`.

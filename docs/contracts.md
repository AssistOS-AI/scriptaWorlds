...
universes/<universe-id>/
  ...
  turns/NNNN.prev/        # snapshot of canon.md, threads.json and atlas.json taken before that turn
...
`language` is the language of the **fiction** of the book (lowercase ISO 639-1 code), chosen at creation, changeable through `POST /api/universes/:id/settings`. The accepted list comes from `GET /api/config`. Schema vocabulary (folder names, plan keys, canon sections, JSON fields, thread kinds and statuses, atlas axes and node states) stays English regardless of the language of the book; only narrative text and edition labels are localized.
...
`kind` ∈ `chapter` | `export`. `status` ∈ `queued` | `running` | `done` | `error` | `interrupted`. `rewrite` is `true` for a rewrite turn and `instructions` then carries the reader's complaint. `warnings` collects non-fatal observations such as a chapter outside the configured word band or a missing offer file.
...
{"ok":true,"title":"The Ash Archive","chapters":3,"words":7412,
 "outputs":[{"format":"pdf","path":"/abs/...pdf","bytes":812345,"pages":96},
            {"format":"docx","path":"/abs/...docx","bytes":38412,"pages":null}]}
```
...
- Exit code `0` on success, `2` on a usage error, `1` on a processing error. Error codes: `USAGE`, `MISSING_UNIVERSE`, `NO_CHAPTERS`, `MISSING_FONT`, `BAD_JSON`, `IO_ERROR`, `INTERNAL`.
- No npm dependencies. Node.js built-ins only. System fonts, with a fallback list.
...
| POST | `/api/universes/:id/chapters/:number/rewrite` | body `{"instructions":"what is wrong and what should change","dropLater":false}` → `202 {"job":Job,"droppedChapters":[3,4]}`. Rewrites chapter `:number` from the canon state before that chapter; the old version is archived in `chapters/.history/`. When later chapters exist and `dropLater` is not `true` → `409 {"error":{"code":"LATER_CHAPTERS","message":"…","laterChapters":[3,4]}}` |
...
- Before every chapter turn the server copies `canon.md`, `threads.json` and `atlas.json` into `turns/NNNN.prev/`. The snapshot is kept after a successful turn because a later rewrite of that chapter needs it; it is consumed only when a state restore happens, either for a rewrite or for the recovery of an interrupted turn.
- After the run the server verifies the result: the chapter file must exist (`NO_CHAPTER`), an export must have produced at least one file (`NO_EXPORT`), and a failed, timed-out or unstarted agent is `AGENT_FAILED`. On failure the state is restored, the partial chapter files are deleted and the turn record is stored with status `error`, keeping the agent's narration tail.
...
### 6.3 First contact: starting from a template
